...
const EditGroup = ({
    ...
    try {
      ...
      video.onloadedmetadata = () => {
        ...
        const calculatedHeight =
          video.videoHeight / (video.videoWidth / calculatedWidth);
  ...
  const removeExistingStudent = (studentNumber, studentName) => {
    if (
      !window.confirm(
        `Weet je zeker dat je ${studentName} wilt verwijderen uit deze groep?`
      )
    )
      return;
  ...
  const saveGroupChanges = async (e) => {
    ...
    const oldStudents = Array.from(oldStudentsRef.current.children).map(
      (student) => ({
    ...
    const newStudentsData = Array.from(newStudentsRef.current.children).map(
      (student) => ({
        name: student.querySelector('input[type="text"]').value,
        student_number: student.querySelector('input[type="number"]').value,
      })
    ...
    const allStudents = [...oldStudents, ...newStudentsData];

    const formData = new FormData();
    formData.append("group_id", group.id);
    formData.append(
      "name",
      document.getElementById("teamName").value || group.name
    );
    formData.append("class", document.getElementById("klas").value || group.class);
    formData.append("students", JSON.stringify(allStudents));
    ...
    if (image) {
      if (typeof image === "string" && image.startsWith("data:")) {
        ...
        formData.append("image", image, "group.png");
      } else if (typeof image === "string") {
        // bestaand image pad gewoon meesturen
        ...
        {camera ? (
          <div className="camera">
            ...
            <div className="buttonWrapper">
              <button type="button" onClick={takePicture} className="btn">
